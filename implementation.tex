...
The methods of position filtering we have implemented can be split into two distinct categories: a pre-correction method and a post-correction method. Using a pre-correction method in the case of disallowing the penetration of objects means to never allow the hand to enter an object by anticipating the penetration in advance. Using a post-correction method in the same case means to correct the hand's position after the fact.

The first method we implemented mainly falls into the post-correction category although it also uses some pre-correction techniques as well.
