...
%Filtering on the player's positional input means to let the position of the hand deviate the controllers position. Reasons for deviating can include to make sure that no objects are penetrated by the hand. Deviating from the controller position means that the player's hand and the hand in VR are not situated at the same position in space, which might lead to confusion for the player (Maybe reference hand visualization here).
...
%Filtering on the rotation input means to rotate the hand differently in the world compared to the rotation of the controller. Rotation filtering can be used as a means to display what is assumed would be the player's intention, when they interact with the world. Using rotation filtering can also reduce the amount of position filtering needed in certain cases.
