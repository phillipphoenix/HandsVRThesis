...
\todo[inline]{Mention that all grabbables implement break forces that make the hand drop objects on impact with high velocities, except the parenting grabbable.}
...
\todo[inline]{A large problem with the parenting grabbable stems from how the different hands move.}

\textbf{\textit{(WRITE ABOUT THE FIXED JOINT GRABBABLE HERE!)}}

\todo[inline]{The Fixed Joint grabbable has the problem that after moving and stopping, the velocities are still present on the grabbed object, making it wobble a bit.}
...
\todo[inline]{This section should be an overall conclusion for the implementation chapter discussing the options and the still unsolved problems.}
