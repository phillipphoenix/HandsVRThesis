...
\todo[inline]{Write a small conclusion for the rigid hand stating the good and the bad things about it. One of the good things about the Rigid hand is that it's very simple to implement as we did. The disadvantages include: using a kinematic rigidbody removes some of the good things that the physics system handles, when using non-kinematic rigidbodies. We have to handle a lot of things by ourselves, like pushing several dynamic objects into each other or into static objects with the hands is very unstable. Collision detection is also manual and can have edge cases that aren't solved.}
