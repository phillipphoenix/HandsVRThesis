...
\todo[inline]{Write a small conclusion for the Sliding rigid hand. This should include what it does well and where it fails. It works very well on flat or rounded surfaces, but has a lot of edge cases, when it comes to concave shapes and edges and corners of objects. Besides this, it has the same disadvantages as the rigidhand, since it's also using a kinematic rigidbody.}
...
\todo[inline]{Write a small conclusion for the Finger rigid hand. This should include what it does well and where it fails. This hand has a lot of the same disadvantages and problems as the rigid hand, on which it is based. Adding the fingers on top adds more edge cases, where the fingers behave weirdly and not as intended or expected.}
...
\missingfigure[figwidth=15cm]{Diagram: Cube example and why the middle direction is important.}
...
\todo[inline]{Write a small conclusion for the Physics hand. This should include what it does well and where it fails. Some of the benefits are already stated above. Some of the cons about out implementation of the Physics hand include: Not handling the rotation explicitly results in a jump, when the distance between the controller and the hand is too large. Furthermore, when moving the fingers, it changes the shape of the rigidbody, which in turn makes the rigidbody move. Because of this the rigidbody needs quite a high value for drag to stop it from shaking.}
