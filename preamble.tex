...
\item Investigated if user experience can be improved by simulating a more realistic behavior, by deviating the hands' positions, rotations and overall pose from the user input.
We introduce three filtering variables: The position, the rotation and the finger positions.
\item Developed five prototypes of hands with different behaviors.
\item Evaluated our best prototype with a baseline mimicking the hands from Job Simulator with a group of testers.
\item Concluded that it's possible to improve the user experience, if certain problems with our prototypes are improved upon.
\end{itemize}
